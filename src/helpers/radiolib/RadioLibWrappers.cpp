...
void RadioLibWrapper::setTxPower(int8_t dbm) {
  _last_tx_power_request_dbm = dbm;
  _last_tx_power_applied_dbm = dbm;
  _last_tx_power_status = _radio->setOutputPower(dbm);
}
...
#ifdef NOISE_FLOOR_SAMPLE_AFTER_AGC_ONLY
  // The E22P receiver can return to its stale high RSSI floor after the AGC
  // recovery window. Keep the last post-reset result instead of replacing it
  // with an unrelated periodic sample. resetAGC() starts the next sample set.
  return;
#endif
...
void RadioLibWrapper::resetNoiseFloorSampling() {
  _noise_floor = 0;
  _raw_noise_floor = 0;
  _num_floor_samples = 0;
  _floor_sample_sum = 0;
...
void RadioLibWrapper::loop() {
  if (state == STATE_RX && _num_floor_samples < NUM_NOISE_FLOOR_SAMPLES) {
    ...
    _raw_noise_floor = _floor_sample_sum / NUM_NOISE_FLOOR_SAMPLES;
    _noise_floor = _raw_noise_floor;
    if (_noise_floor < -120) {
